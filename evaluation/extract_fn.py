...
def  extract_features(dataloader, model, device, scales):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader):
            ...
            features.append(_multi_scale(model, inputs, scales)) #g: , inputs2 (for 2 branches)

    features = np.concatenate(features, axis=0)
    return features


def _single_scale(model, inputs, scale): #g: , inputs2 (for 2 branches)
    """
    Args:
        inputs: (B, 3, H, W)
    """
    ...
    with torch.no_grad():
        ...
        #features = torch.cat((features, features2), 1) #g: concatenate features of the 2 branches
    return features.cpu().numpy()


def _multi_scale(model, inputs, scales): #g: , inputs2 (for 2 branches)
    features = None
    for scale in scales:
        _features = _single_scale(model, inputs, scale) #g: , inputs2 (for 2 branches)
